...
def _status(message: str) -> None:
    print(f"[vectorize-mailbox] {message}", file=sys.stderr)
...
def _build_progress_label(normalized_email) -> str:
    subject_label = _display_label(normalized_email.subject)
    if normalized_email.folders:
        return f"[{','.join(normalized_email.folders)}] {subject_label}"
...
def _render_progress(
    ...
    *,
    ...
    message = (
        f"\r[vectorize-mailbox] Processing [{progress_bar}] {current}/{total} "
...
def _normalize_emails(email_messages, normalizer: EmailNormalizer):
    normalized_emails = [normalizer.normalize(email_message) for email_message in email_messages]
    return normalizer.merge_duplicates(normalized_emails)
...
def main() -> int:
    ...
    normalized_emails = _normalize_emails(emails, normalizer)
    if len(normalized_emails) != len(emails):
        _status(
            "Collapsed duplicate messages across folders into unique persistence records. "
            f"raw={len(emails)} unique={len(normalized_emails)}"
        )
    if not args.dry_run:
    ...
    fetched_count = len(normalized_emails)
    inserted_count = 0
    updated_count = 0
    skipped_count = 0

    for index, normalized_email in enumerate(normalized_emails, start=1):
        subject_label = _build_progress_label(normalized_email)
        try:
